...
### `admin_profiles` (admins shown publicly)
- `user_id BIGINT PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE`
- `full_name TEXT NOT NULL`
- `avatar_url TEXT`
- `bio TEXT`
...
Notes:
- An admin profile should have `users.is_admin=true`.
- `admin_profiles.is_public` controls visibility on the website.
...
## 6) Projects (Student Portfolio / Website Showcase)
...
Indexes (recommended):
- `(student_user_id, sort_order)`
- `(is_public, sort_order, created_at)`

---

## 7) Forms (Dynamic Fields)
...
## 8) Applications (Visitor applies to cohort)
...
## 9) Enrollments (Created by admin approval)
...
## 10) Announcements (can appear on website)
...
## 11) Contact Messages (Visitors + Companies/Recruiters Visit Requests)
...
# 12) NEW: Logs + Admin Notifications ✅
...
## 12.3 How logs → notifications works (implementation note)
