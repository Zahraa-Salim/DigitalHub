// File: frontend/src/components/inner-pages/instructors/InstructorArea.tsx
// Purpose: Renders the instructor area UI block for the frontend.
...
export default function InstructorArea() {
    ...
    const fetchTeam = async () => {
      try {
        const [managersRes, instructorsRes] = await Promise.all([
          fetch(`${API_BASE_URL}/public/managers`, { cache: "no-store", credentials: "omit" }),
          fetch(`${API_BASE_URL}/public/instructors`, { cache: "no-store", credentials: "omit" }),
        ]);
  ...
  const photoSrc = (m: TeamMember) => {
    ...
    return `${API_BASE_URL}${m.photoUrl}`;
  };
