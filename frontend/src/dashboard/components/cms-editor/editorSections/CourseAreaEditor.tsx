...
import {
  DEFAULT_PROGRAM_CARD_STYLE,
  resolveProgramCardStyle,
  type ProgramCardStyle,
} from "@/lib/programCardStyle";
import { EditableSpan } from "../EditableSpan";

type CourseAreaEditorProps = {
  style?: boolean;
...
export default function CourseAreaEditor({ style, content, sectionId }: CourseAreaEditorProps) {
  ...
  const bindNavigation = () => {
    ...
    const navigationParams = typeof swiper.params.navigation === "boolean" ? {} : swiper.params.navigation;
  ...
  useEffect(() => {
    ...
    const loadCohorts = async () => {
      try {
        ...
        if (!active) return;
        setPrograms(mapOpenCohortsToProgramCards(cohorts).slice(0, maxCards));
        setLoadError(null);
      } catch {
        if (!active) return;
  ...
  const filteredPrograms =
    !showStatusTabs || activeTab === 0
      ? programs
      : programs.filter((program) => program.status === statusFilters[activeTab - 1]?.value);
  ...
  const getProgramIconClass = (programName: string) => {
    ...
    if (normalized.includes("design") || normalized.includes("ui") || normalized.includes("ux")) return "fas fa-palette";
    if (normalized.includes("data") || normalized.includes("analytics") || normalized.includes("analysis")) return "fas fa-chart-line";
    if (normalized.includes("ai") || normalized.includes("ml") || normalized.includes("machine")) return "fas fa-robot";
    if (normalized.includes("marketing") || normalized.includes("media")) return "fas fa-bullhorn";
    if (normalized.includes("network") || normalized.includes("security")) return "fas fa-shield-alt";
    return "fas fa-code";
  };
      ...
      <div className="container">
          ...
          <div className="row justify-content-center">
            <div className="col-lg-6">
              <div className="section__title text-center mb-40">
                <EditableSpan sectionId={sectionId} field="subtitle" fallback="Programs" tag="span" className="sub-title" />
                <EditableSpan
                  sectionId={sectionId}
                  field="title"
                  fallback="Explore Current and Upcoming Programs"
                  tag="h2"
                  className="title"
                />
                <p className="desc">
                  <EditableSpan sectionId={sectionId} field="description" fallback={sectionDescription} multiline />
                </p>
              </div>
                ...
                <ul className="nav nav-tabs">
                  ...
                  {showStatusTabs && statusFilters.map((status, index) => (
                    <li key={status.value} onClick={() => setActiveTab(index + 1)} className="nav-item">
                      <button className={`nav-link ${activeTab === index + 1 ? "active" : ""}`} type="button">
                        {status.label}
                      </button>
                    </li>
                  ))}
        ...
        {!showSkeletonCards && filteredPrograms.length === 0 && (
          <div className="text-center mt-30">
            <p>
              <EditableSpan sectionId={sectionId} field="empty_state_text" fallback={emptyStateText} />
            </p>
          </div>
        )}
        ...
        <div className="dh-programs-more">
          <Link to={moreButtonLink} className="btn">
            <EditableSpan sectionId={sectionId} field="more_button_text" fallback="See More Programs" />
          </Link>
        </div>
      </div>
    </section>
