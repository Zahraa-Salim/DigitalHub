...
export type PublicCohortPerson = {
  user_id: number;
  full_name: string;
...
export type PublicCohortDetail = PublicCohort & {
  instructors: PublicCohortPerson[];
  students: PublicCohortPerson[];
};
...
export async function getPublicCohortDetail(cohortId: number) {
  return getData<PublicCohortDetail>(`/public/cohorts/${cohortId}`);
}
...
export async function getPublicThemeTokens() {
  return getData<PublicThemeToken[]>("/public/theme");
}

export async function getPublicPageByKey(pageKey: string) {
  const normalized = String(pageKey || "").trim();
  if (!normalized) {
    throw new Error("Page key is required.");
  }
  return getData<PublicPage>(`/public/pages/${encodeURIComponent(normalized)}`);
}
...
export async function listPublicAnnouncements(params?: {
  ...
  return getList<PublicAnnouncement>("/public/announcements", {
    page: params?.page ?? 1,
