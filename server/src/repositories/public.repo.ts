...
export async function findProgramApplicationByEmailNorm(programId, emailNorm, db = pool) {
    return db.query(`
      SELECT id
      FROM program_applications
      WHERE program_id = $1
        AND applicant_email_norm = $2
      LIMIT 1
    `, [programId, emailNorm]);
}

export async function findProgramApplicationByPhoneNorm(programId, phoneNorm, db = pool) {
    return db.query(`
      SELECT id
      FROM program_applications
      WHERE program_id = $1
        AND applicant_phone_norm = $2
      LIMIT 1
    `, [programId, phoneNorm]);
}
